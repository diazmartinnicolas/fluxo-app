...
function App() {
  ...
  const categories = ['Todo', 'Promociones', 'Pizzas', 'Milanesas', 'Hamburguesas', 'Empanadas', 'Ensaladas', 'Mitades', 'Bebidas', 'Postres', 'Otros'];
  ...
  const filteredProducts = products
    ...
    .filter(p => productSearchTerm === '' || p.name.toLowerCase().includes(productSearchTerm.toLowerCase())) // Búsqueda por nombre
          ...
          <div className="flex h-full flex-col md:flex-row">
            <div className={`flex-1 overflow-y-auto p-4 md:p-6 ${mobileView === 'cart' ? 'hidden md:block' : 'block'}`}>
              <header className="flex gap-2 mb-4 overflow-x-auto pb-2 no-scrollbar">
                {categories.map(cat => (
                  <button key={cat} onClick={() => { setSelectedCategory(cat); setProductSearchTerm(''); }} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${selectedCategory === cat ? 'bg-orange-600 text-white shadow-md' : 'bg-white border text-gray-600'}`}>{cat}</button>
                ))}
              </header>
              ...
              <div className="relative mb-4">
                ...
                <input
                  ...
                  className="w-full pl-10 p-3 border border-gray-200 rounded-xl text-sm bg-white shadow-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 outline-none transition-all"
                  value={productSearchTerm}
                  onChange={e => setProductSearchTerm(e.target.value)}
                />
                {productSearchTerm && (
                  <button onClick={() => setProductSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                    <X size={16} />
                  </button>
                )}
              </div>
